...
def thumbnail_card(item):
    contents = GalleryItemsByCollectionPermission.objects.filter(gc_uuid=item.contents_json["col"],gci_uuid=item.contents_json["item"],gif_thumbnail=True).order_by('-gif_size')[0]
