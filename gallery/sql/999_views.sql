...
SELECT DISTINCT ON (gallery_galleryitemfile.hash) hash as gci_hash,
    gallery_galleryfavourite.profile_id AS gif_profile,
    gallery_galleryitem.title AS gci_title,
    gallery_galleryitem.descr AS gci_descr,
    gallery_galleryitem.created AS gci_created,
    gallery_galleryitem.updated AS gci_updated,
    ...
    profiles_profile.uuid AS profile_uuid,
    profiles_profile.uuid AS profile_id,
    gallery_galleryfavourite.id AS id,
    substring(profiles_profile.uuid::text from 1 for 1)||'/'||substring(profiles_profile.uuid::text from 2 for 1)||'/'||substring(profiles_profile.uuid::text from 3 for 1) AS gal_path,
    ...
    FROM gallery_galleryfavourite
     JOIN gallery_galleryitem ON gallery_galleryfavourite.item_id = gallery_galleryitem.uuid
     JOIN gallery_galleryitemfile ON gallery_galleryitem.uuid = gallery_galleryitemfile.item_id
     JOIN profiles_profile ON profiles_profile.uuid = gallery_galleryitem.profile_id
  WHERE gallery_galleryitemfile.name <> ''::text  AND gallery_galleryitemfile.thumbnail = true;
