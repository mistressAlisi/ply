...
from communities.community.models import CommunityAdmins,CommunityProfileDashboardRoles,CommunityStaff
...
def create_community_staff(request):
    ...
    if form.data["profile"] == "":
        if not form.is_valid():
            ...
            form.save()
            # Add Dashboard:
            db = CommunityProfileDashboardRoles.objects.get_or_create(profile=form.instance.profile,type__type='staff',community=community)[0]
            db.save()
...
def delete_community_staff(request,staff):
    ...
    dashobj = CommunityProfileDashboardRoles.objects.get(profile=staffobj.profile, community=staffobj.community,type__type='staff')
    dashobj.delete()
    staffobj.delete()
