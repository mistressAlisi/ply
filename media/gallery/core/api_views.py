...
from media.gallery.core.models import GalleryTempFile, GalleryCatGroupObject,GalleryCollectionPermission,GalleryItem,GalleryCollection,GalleryTempFileThumb,GalleryCollectionItems,GalleryItemFile,GalleryItemKeyword,GalleryFavourite
...
def gallery_recast_item(request,col,item):
    ...
    col.shares = item.shares + 1;
    col.save();
    message = stream_toolkit.post_to_active_profile(request,"application/ply.stream.core",'<i class="fa-solid fa-retweet"></i> Recasted from Gallery!',{"col":str(col.uuid),"item":str(item.uuid)})
    return JsonResponse("ok",safe=False)
...
def gallery_copymove_exec(request):
    ...
    if "cast_to_stream" in request.POST:
        if (request.POST["cast_to_stream"] == "on"):
            message = stream_toolkit.post_to_active_profile(request,"application/ply.stream.core",'<i class="fa-solid fa-retweet"></i> Casted from Gallery!',{"col":str(col.uuid),"item":str(item.uuid)})
    return JsonResponse("ok",safe=False)
...
def gallery_toggle_fav(request,item):
    ...
    if (fav.exists()):
        ...
        noti = Notification.objects.create(source=profile,community=community,type="ply.core.fav_created",contents_text=str(item.uuid))
        noti.save()
        # Create "Someone faved your art" notifcation:
        noti = Notification.objects.create(source=item.profile,community=community,type="ply.core.item_favd",contents_text=str(item.uuid))
        noti.save()
