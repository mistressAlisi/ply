...
log = plylog.getLogger('core.tasks',name='core.tasks')
app = Celery('ply')
...
def publish_to_gallery(data,profile,file_plugin,temp_file,user,community,metadata={}):
    ...
    try:
        ...
        try:
            ...
            # TODO: implement :
            # Step Eight: Streams!
            streams_toolkit.post_to_profile_stream(profile,community,"application/ply.stream.core",'<i class="fa-solid fa-retweet"></i> Recasted from Gallery!',{"col":str(col.uuid),"item":str(item.uuid)})
            ...
            print(e)
            log.error(f"File Uploader: Unable to generate Publish file {temp_file} to core: {e}")
            logging.exception(e)
