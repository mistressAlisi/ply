...
def regForm1(request):
    ...
    context = {'regform':regform,'condform':condform,'sessionRegs':sessionRegs,'sessionRegCount':regcount,'sessionTotal':sessionTotal,'event':event_data.event,'community':community,'active_1':True,'sessionLoot':sessionLoot,'lootTotal':lootTotal}
...
def merchForm1(request):
    ...
    if request.user.is_authenticated:
        regform.load_profiles(community,request.user)
    condform = ConditionsForm()
    sessionRegs = []
    sessionTotal = Decimal(0)
